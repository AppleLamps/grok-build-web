...
- **[done via Settings] Agent / subagent definitions** — `[flag --agent]` `[flag --agents <JSON>]` Settings panel exposes primary agent selection and inline subagent JSON, with server-side JSON validation before respawn.
...
- **[DONE] Compaction visibility and controls** — `[slash /compact]` `[slash /context]` `[flag --compaction-mode]` `[flag --compaction-detail]` Settings exposes supported compaction launch flags when the installed CLI advertises them, and web surfaces compaction result metadata in the transcript plus Tools -> Session info when the agent emits it.
