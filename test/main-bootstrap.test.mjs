...
test('main bootstrap adopts explicit session URLs and loads cwd metadata', async () => {
    ...
    fetchImpl: async (url, opts = {}) => {
      ...
      if (String(url).startsWith('/sessions')) {
        return new Response(JSON.stringify({
          sessions: [{ id: 'session-2', cwd: 'C:\\Users\\apple\\project' }],
        }), { status: 200 });
      }
  ...
  assert.deepEqual(calls[1], {
    url: '/tab/load',
    body: { sessionId: 'session-2', cwd: 'C:\\Users\\apple\\project' },
  });
  assert.equal(calls[2].url, '/session/plan?sessionId=session-2&cwd=C%3A%5CUsers%5Capple%5Cproject');
