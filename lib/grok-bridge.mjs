import { AGENT_IDLE_MS, AGENT_IDLE_SWEEP_MS, CWD, HISTORY_LIMIT, MAX_ACTIVE_AGENTS } from './config.mjs';
...
export class GrokBridge {
  ...
  createAgentConnection() {
    return new AgentConnection({
      ...
      onActivity: (sessionId) => {
        if (sessionId)
          this.sessionCwds.set(sessionId, this.agents.get(sessionId)?.cwd ?? this.sessionCwds.get(sessionId));
      },
      onExit: (sessionId) => {
  ...
  prompt(text, sessionId = null, opts = {}) {
    ...
    if (!opts.internal) {
      ...
      this.broadcast({ kind: 'user_prompt', text, attachments, sessionId: target, turnId });
    }
  ...
  respondToPermission(rpcId, optionId, sessionId = null) {
    ...
    return this.agents.get(sessionId)?.respondToPermission(rpcId, optionId) ?? false;
  }

  respondToElicitation(rpcId, action, content, sessionId = null) {
    if (!sessionId) return false;
    return this.agents.get(sessionId)?.respondToElicitation(rpcId, action, content) ?? false;
  }
  ...
  async gracefulShutdown(timeoutMs) {
    clearInterval(this.idleSweepTimer);
    const agents = [...this.agents.values()];
    if (!agents.length) return;
    const hasInFlight = agents.some((a) => a.activeTurn || a.turnQueue.length > 0);
    if (hasInFlight) {
      ...
      while (Date.now() < deadline) {
        if (!agents.some((a) => a.activeTurn)) break;
        await new Promise((r) => setTimeout(r, 50));
      }
    }
    await Promise.all(agents.map((agent) => agent.killChild()));
    this.agents.clear();
  }
  ...
  replayEntries(sessionIdFilter, { sinceSeq = null, replayAll = true } = {}) {
    const since = Number.isFinite(sinceSeq) ? sinceSeq : null;
    if (!replayAll && since === null) return [];
    if (!sessionIdFilter) {
      return since === null ? this.history : this.history.filter((entry) => entry.seq > since);
    }
    const sessionEntries = this.sessionHistory.get(sessionIdFilter) ?? [];
    const merged = mergeHistoryEntries(this.globalHistory, sessionEntries);
    return since === null ? merged : merged.filter((entry) => entry.seq > since);
  }
