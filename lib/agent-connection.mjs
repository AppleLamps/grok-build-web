...
import { terminateChildProcess } from './process-cleanup.mjs';
import { buildAgentArgv } from './spawn-opts.mjs';
import { StderrRateLimiter } from './stderr-limiter.mjs';
import { cleanDisplayPath, errorMessage, isMissingCwdError, isWithinPath, rpcErrorMessage } from './util.mjs';

function buildPromptText(text, attachments, prefix = '') {
  let promptText = text;
  if (prefix) {
    const trimmedPrefix = prefix.replace(/\s+$/, '');
    promptText = `${trimmedPrefix}\n\n${text ?? ''}`;
  }
  if (!Array.isArray(attachments) || attachments.length === 0) return promptText;
  const lines = attachments
    .map((a) => (typeof a?.path === 'string' && a.path ? `- ${cleanDisplayPath(a.path)}` : null))
    .filter(Boolean);
...
export class AgentConnection {
  constructor({ emit, spawnOpts, autoApproveFor, onActivity = () => {}, onExit = () => {}, now = () => new Date() }) {
    this.emit = emit;
    ...
    this.lastPromptLocalDate = localDateStamp(this.now());
  }
  ...
  async killChild() {
    ...
    await terminateChildProcess(dying, { graceMs: CHILD_KILL_GRACE_MS });
  }
    ...
    if (msg.method === 'session/request_permission' && msg.id !== undefined) {
      ...
      if (this.autoApproveFor(sessionId)) {
        ...
        if (optionId) {
          ...
          this.emit({
            kind: 'permission_auto_cancelled',
            reason: 'no_options',
  ...
  send(obj) {
    ...
    try {
      ...
      console.error('[grok-web] stdin write failed:', errorMessage(e));
      return false;
    }
  }
  ...
  async resolveClientPath(params = {}, { forWrite = false } = {}) {
    ...
    if (!isWithinPath(root, requested)) throw new Error('path outside session cwd');
  ...
  prompt(text, opts = {}) {
    ...
    if (!turn.internal) this.emit({ kind: 'user_prompt', text, attachments, sessionId: target, turnId: turn.turnId });
  ...
  async runQueuedPrompt(turn) {
    ...
    this.touch();
    try {
      const dateRolloverNotice = turn.internal ? '' : this.takeDateRolloverNotice();
      const res = await this.call(
        'session/prompt',
        {
          sessionId: this.sessionId,
          prompt: [{ type: 'text', text: buildPromptText(turn.text, turn.attachments, dateRolloverNotice) }],
        },
  ...
  takeDateRolloverNotice() {
    const current = localDateStamp(this.now());
    if (!current) return '';
    const previous = this.lastPromptLocalDate;
    if (!previous) {
      this.lastPromptLocalDate = current;
      return '';
    }
    if (current === previous) return '';
    this.lastPromptLocalDate = current;
    const message = `System notice: The local calendar date changed from ${previous} to ${current} while this session was open. Treat the current local date as ${current}.`;
    this.emit({
  ...
  respondToPermission(rpcId, optionId) {
    ...
    const outcome = optionId === '__cancel__' ? { outcome: 'cancelled' } : { outcome: 'selected', optionId };
  ...
  respondToElicitation(rpcId, action, content) {
    ...
    const result =
      pending.responseKind === 'ask_user_question'
