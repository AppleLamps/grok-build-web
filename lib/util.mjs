...
export function positiveIntegerOption(value, name) {
  ...
  const ok =
    (typeof value === 'number' && Number.isInteger(value)) || (typeof value === 'string' && /^\d+$/.test(value.trim()));
...
export function defaultUsername() {
  return process.env.GROK_WEB_USER ?? process.env.USERNAME ?? process.env.USER ?? userInfo().username ?? 'local';
}
