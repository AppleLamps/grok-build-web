...
export function match(method, pathname) {
  return (method === 'POST' && pathname === '/upload') || (method === 'GET' && pathname === '/upload-media');
}
...
function sendUploadResult(res, target, size) {
  const ext = extname(target.stored).toLowerCase();
  const mediaUrl = MEDIA_MIME[ext]
    ? `/upload-media?sessionId=${encodeURIComponent(target.sessionId)}&name=${encodeURIComponent(target.stored)}`
    : null;
  const body = {
    ok: true,
