...
function requestSessionId(body) {
  return typeof body?.sessionId === 'string' && body.sessionId.trim() ? body.sessionId.trim() : null;
}
...
export async function handle(ctx) {
  ...
  if (pathname === '/permission') {
    ...
    try {
      ...
      if (!sessionId) {
        sendJsonErr(res, 400, 'sessionId required'); return true;
      }
      const ok = grok.respondToPermission(body.rpcId, body.optionId, sessionId);
  ...
  if (pathname === '/elicitation') {
    ...
    try {
      ...
      const ok = grok.respondToElicitation(body.rpcId, body.action, body.content, sessionId);
      if (!ok) {
