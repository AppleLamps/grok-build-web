...
export function match(method, pathname) {
  return (
    ...
    (pathname === '/' ||
      ...
      pathname === '/session-media' ||
      pathname === '/vendor/mermaid.min.js')
  );
}
...
export async function handle(ctx) {
  ...
  if (req.method === 'GET' && url.pathname === '/vendor/mermaid.min.js') {
    ...
    try {
      const file = resolve(PUBLIC_DIR, '..', 'node_modules', 'mermaid', 'dist', 'mermaid.min.js');
      const data = await readFile(file);
      res.writeHead(200, { 'content-type': MIME['.js'] ?? 'text/javascript; charset=utf-8' });
      res.end(data);
    } catch {
