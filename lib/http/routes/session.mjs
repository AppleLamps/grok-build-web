...
import { isRequestBodyTooLarge, readBody, sendJson, sendJsonError } from '../response.mjs';
...
export async function handle(ctx) {
  ...
  if (req.method === 'POST' && pathname === '/session/new') {
    ...
    try {
      ...
      const tab = await grok.newSession(body.cwd);
      sendJson(res, 200, { sessionId: tab.sessionId, cwd: tab.cwd });
    } catch (e) {
  ...
  if (req.method === 'POST' && pathname === '/session/load') {
    ...
    try {
      ...
      const tab = await grok.loadSession(body.sessionId, body.cwd, { restoreCode: !!body.restoreCode });
      sendJson(res, 200, { sessionId: tab.sessionId, cwd: tab.cwd });
    } catch (e) {
      if (isRequestBodyTooLarge(e)) { sendJsonError(res, 400, e); return true; }
      sendJsonError(res, e?.status ?? 500, e);
